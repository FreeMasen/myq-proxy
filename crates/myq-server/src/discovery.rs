...
pub async fn discovery_task(ip: Ipv4Addr, port: u16) {
    log::debug!("starting discovery task {ip}:{port}");
    ...
    loop {
        ...
        log::debug!(
            "req({ct}, {who}): {:?}",
            String::from_utf8_lossy(&buf[..ct])
        );
        if let Ok(msg) = serde_json::from_slice::<QueryMessage>(&buf[..ct])
            .map_err(|e| log::debug!("bad query: {e}"))
        {
            if msg.target == "myq-proxy" {
                log::info!("handling disovery request from {}", msg.source);
                udp.send_to(&serde_json::to_vec(&QueryReply { ip, port }).unwrap(), who)
                    .await
                    .unwrap();
            }
        }
...
async fn get_sock() -> UdpSocket {
    ...
    udp.join_multicast_v4(&MULTICAST_ADDR, &Ipv4Addr::new(0, 0, 0, 0))
        .unwrap();
...
mod test {
    ...
    async fn replies() {
        ...
        let send_ct = udp
            .send_to(&query_bytes, (MULTICAST_ADDR, MULTICAST_PORT))
            .await
            .unwrap();
